        ...
        impl BlockCipher for $name {
            ...
            fn new(key: &GenericArray<u8, $key_size>) -> Self {
                ...
                for i in 0..$rounds {
                    ...
                    c.enc_keys8[i] =
                        bit_slice_fill_4x4_with_u32x4(ek[i][0], ek[i][1], ek[i][2], ek[i][3]);
                    c.dec_keys8[i] =
                        bit_slice_fill_4x4_with_u32x4(dk[i][0], dk[i][1], dk[i][2], dk[i][3]);
                }
                c
            }
            ...
            fn encrypt_blocks(&self, blocks: &mut Block128x8) {
                let blocks: &mut [u8; 16 * 8] = unsafe { mem::transmute(blocks) };
